...
export function CreateAgentForm({onSuccess}:CreateAgentFormProps) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setSuccess(false);
    try {
      const response = await AxiosInstance.post("/api/v1/agent",
        {
          name,
          sourceUrl,
        },
      ...
      <Card className="overflow-hidden pt-1 gap-2 bg-white border-none shadow-none">
        <CardHeader className="pb-4 px-0">
          <CardDescription className="text-slate-500 px-0">
            Fill in the details below to create a custom AI agent for your
            website
          </CardDescription>
        </CardHeader>
        ...
        <CardFooter className="bg-slate-50 border-t border-slate-100 px-6 py-4 mt-3">
          <p className="text-xs text-slate-500 text-center w-full">
            Your agent will be ready to answer questions about your website
            content after crawling is complete
          </p>
        </CardFooter>
      </Card>
    </div>
